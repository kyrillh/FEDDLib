...
void NonLinElasticity<SC,LO,GO,NO>::reAssemble(std::string type) const {
    ...
    if (type=="Newton-Residual") {
        ...
        if(useInterface && material_model == "Neo-Hooke"){
...
template <class SC, class LO, class GO, class NO>
void NonLinElasticity<SC, LO, GO, NO>::reInitSpecificProblemVectors(const Teuchos::RCP<const BlockMap<LO, GO, NO>> newMap) {
